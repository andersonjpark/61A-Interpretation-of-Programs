...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        rest = rest.map(eval_operands)

        return scheme_apply(operator, rest, env) # why not operator.apply?
        # END PROBLEM 4
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(vals) != len(formals):
            raise SchemeError("The length of values and formals are not equal")
...
def do_quasiquote_form(expressions, env):
    ...
    def quasiquote_item(val, env, level):
        ...
        if not scheme_pairp(val):
            return val
        if val.first is 'unquote':
            level -= 1
            if level == 0:
                expressions = val.rest
                check_form(expressions, 1, 1)
                return scheme_eval(expressions.first, env)
        elif val.first is 'quasiquote':
            level += 1
