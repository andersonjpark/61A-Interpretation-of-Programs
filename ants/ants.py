...
class Ant(Insect):
    ...
    def can_contain(self, other):
        return self.is_container and not other.is_container and not self.is_ant
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        def bee_finder(place, beehive):
            nonlocal range_counter

            if place == beehive:
                return None

            elif range_counter > max_range:
                return None

            elif (place.bees and place != beehive) and (min_range <= range_counter <= max_range):
                return random_or_none(place.bees)

            else:
                range_counter += 1
                return bee_finder(place.entrance, beehive)

        return bee_finder(place, beehive)
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        for bee in list(bees):
            bee.reduce_armor(amount) # Apply damage to each bee in the current place

        Ant.reduce_armor(self, amount)

        if self.armor <= 0: # apply the bonus if the fire ant dies
            for bee in list(bees):
    ...
    def action(self, colony):
        ...
        if self.impostor:
            ...
            while place.exit:
                ...
                if isinstance(ant, BodyguardAnt) and ant.contained_ant and ant not in self.double_ants:
